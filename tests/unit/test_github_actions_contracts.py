...
ADVERSARIAL_REVIEW_WORKFLOW = (
    ROOT / ".github" / "workflows" / "agentops-adversarial-pr-review.yml"
)
...
def test_cross_platform_workflow_covers_target_operating_systems() -> None:
    ...
    assert _matrix_values(backend, "os") == [
        "ubuntu-latest",
        "macos-latest",
        "windows-latest",
    ]
    assert _matrix_values(frontend, "os") == [
        "ubuntu-latest",
        "macos-latest",
        "windows-latest",
    ]
...
def test_cross_platform_workflow_covers_backend_gates_in_backend_job() -> None:
    ...
    assert "          python-version: ${{ matrix.python-version }}" in _step_block(
        backend, "Set up Python"
    )
    assert "        run: uv sync --locked" in _step_block(
    ...
    assert "        run: uv build --sdist --wheel --out-dir dist/python" in _step_block(
        backend, "Build Python package"
    )
...
def test_cross_platform_workflow_covers_frontend_gates_in_frontend_job() -> None:
    ...
    assert "          node-version: ${{ matrix.node-version }}" in _step_block(
        frontend, "Set up Node"
    )
    assert (
        "          cache-dependency-path: apps/agentops-console/package-lock.json"
        in _step_block(frontend, "Set up Node")
    )
    assert "        run: npm ci --audit=false" in _step_block(
...
def test_cross_platform_workflow_uploads_per_platform_packages() -> None:
    backend_upload = _block_text(
        _step_block(_job_block("backend"), "Upload Python package artifact")
    )
    frontend_upload = _block_text(
        _step_block(_job_block("frontend"), "Upload console package artifact")
    )

    assert "uses: actions/upload-artifact@v4" in backend_upload
    assert (
        "name: agentops-python-${{ matrix.os }}-py${{ matrix.python-version }}"
        in backend_upload
    )
    ...
    assert (
        "name: agentops-console-${{ matrix.os }}-node${{ matrix.node-version }}"
        in frontend_upload
    )
...
def test_cross_platform_engineering_constraints_are_documented() -> None:
    assert ENGINEERING_DOC.exists(), (
        "Cross-platform engineering constraints must be documented"
    )
...
def test_adversarial_pr_review_workflow_posts_review_comments() -> None:
    assert ADVERSARIAL_REVIEW_WORKFLOW.exists(), (
        "AgentOps adversarial PR review workflow must exist"
    )
...
def test_adversarial_pr_review_script_enforces_agentops_redlines() -> None:
    assert ADVERSARIAL_REVIEW_SCRIPT.exists(), (
        "AgentOps adversarial review script must exist"
    )
    ...
    assert (
        "test_ao21_ct_003c_reissue_bad_handoff_parse_error_rolls_back_session" in script
    )
