...
def test_active_grant_can_be_consumed(repository):
    ...
    consumption = consume_grant(grant["grant_id"], policy_request(policy_check_id=grant["policy_check_id"]), repository)
...
def test_revoked_grant_cannot_be_consumed(repository):
    ...
    with pytest.raises(AgentOpsError) as exc:
        consume_grant(grant["grant_id"], policy_request(policy_check_id=grant["policy_check_id"]), repository)
