...
- 多次轮换时，revoked ancestor 会沿 replacement chain 解析最新 active token，避免误拒最新替代凭证并继续拒绝 stale token。
- malformed handoff parse error 会清理未签发 reissue session，修正 payload 后可重试。
