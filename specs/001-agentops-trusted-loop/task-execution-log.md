...
- 契约测试：AO-CT-001 到 AO-CT-006 均通过；新增 enterprise source/credential/device 状态校验、BootstrapSession 状态/过期、installation/user/session 绑定、幂等重试仍需 assertion/device proof/nonce proof 校验、device proof 过期/签名约束、timestamp skew 和 nonce replay 测试。
