...
from agentops.core.envelope import (
    SIGNATURE_TEST_EVENT_TYPE,
    evidence_mode_for,
    ...
    for event in events:
        ...
        try:
            ...
            if event["event_type"] == SIGNATURE_TEST_EVENT_TYPE:
                signature_test_bootstrap_id = repository.validate_signature_test_event(
                    event
                )
            outcome = repository.write_event(
                event, evidence_mode=evidence_mode_for(event)
            )
            if outcome == "deduplicated":
                ...
                if signature_test_bootstrap_id is not None:
                    repository.mark_signature_test_verified(
                        signature_test_bootstrap_id, event["event_id"]
                    )
